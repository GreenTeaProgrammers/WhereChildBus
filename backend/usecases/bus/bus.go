	...
	for index, guardianID := range guardianIDs {
		...
		if err != nil {
			return fmt.Errorf("failed to find station for guardian ID '%s': %w", guardianID, err)
                                ...
                                // 既存のnextStationリレーションをクリアします
                err = tx.Station.UpdateOne(currentStation).
                    ClearMorningNextStation(). // 朝のnextStationリレーションをクリア
                    ClearEveningNextStation(). // 夕方のnextStationリレーションをクリア
                    Exec(ctx)
                if err != nil {
                    return fmt.Errorf("failed to clear next stations for station ID '%s': %w", currentStation.ID, err)
                }
